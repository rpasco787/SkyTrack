...
The same stack runs on a local [kind](https://kind.sigs.k8s.io) cluster from plain manifests in
`deploy/k8s/` (one Deployment + ClusterIP Service per compose service, assembled by
`kustomization.yaml`; no Helm):

```sh
brew install kind                 # kubectl is assumed
./scripts/kind-up.sh              # builds the image, creates the cluster, applies, port-forwards
./scripts/kind-smoke.sh           # deletes the app pod, checks it is recreated and readiness gates traffic
./scripts/kind-down.sh
```

`kind-up.sh` leaves the API on <http://localhost:8080> and Grafana on <http://localhost:3000>
(forwarders reconnect when a pod is replaced). `SKYTRACK_IMAGE_SOURCE=local` reuses an image
already tagged `ghcr.io/rpasco787/skytrack:latest` in Docker, `pull` fetches it from GHCR;
the default builds it.

Differences from compose: the files compose bind-mounts (`init-aws.sh`, WireMock stubs,
`prometheus.yml`, Grafana provisioning) are committed ConfigMaps regenerated by
`scripts/k8s-gen-configmaps.sh`; the datasets are mounted into the kind node
(`deploy/kind/cluster.yaml`) and into the pod with `hostPath`; the app reads the committed BTS
fixture rather than the 196 MB CSV; the gitignored `wiremock/generated` stubs are not loaded.
The `skytrack` Deployment starts behind an init container that waits for LocalStack's init
script, and its readiness/liveness probes are `/actuator/health/readiness` and `/liveness`.
